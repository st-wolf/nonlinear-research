...
%
\begin{figure}[Ht!]
\center{\includegraphics[width=1\linewidth]{pic/V_1period_shifted.eps}}
\caption{На 1-ом периоде функций Якоби ($[-2K; 2K]$) после сдвига}
\end{figure}
%
%\begin{figure}[Ht!]
%\center{\includegraphics[width=1\linewidth]{pic/V_1period_shifted_asym.eps}}
...
%
\begin{equation}
V(x) = -\lambda (1 - \lambda) (2x^2 - 8(1 - 2\lambda)x^4 + o(x^4)).
\end{equation}
%
...
Случай ``$-$'' не означает, что функция $\tau(E)$ монотонна, однако мы, в след за работой Чудновского, будем полагать ее в этом случае монотонной (т.е. что ее локальное поведение можно экстраполировать на глобальное).
...
%
\begin{figure}[Ht!]
\center{\includegraphics[width=1\linewidth]{pic/tau_on_E.eps}}
\caption{$\tau(E)$}
\end{figure}
%

\paragraph{Rabi: $\Lambda \gg 1$.}
Выражение для потенциала:
%
\begin{equation}
V(x) = \frac{\frac{1}{4} \Lambda^2 ~\sn^2 x - \Lambda~\cn x}{\dn^2 x}.
\end{equation}
%
В этом случае нет симметричной double-well ловушки.

\paragraph{Период малых колебаний}
Рассмотри малые колебания в яме: $\sn~x \sim x$, $\cn~x \sim 1$, $\dn~x \sim 1$.
